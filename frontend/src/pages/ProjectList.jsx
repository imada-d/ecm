...
import { Edit2, Trash2, X, ChevronDown, ChevronUp, ArrowUpDown, ArrowUp, ArrowDown, Printer } from 'lucide-react';
...
function ProjectList() {
  ...
  const getFilteredProjects = () => {
    ...
    console.log('showGeneralExpense:', showGeneralExpense);
    console.log('全工事:', filtered);

        // 全体経費のフィルタリング
    if (!showGeneralExpense) {
      filtered = filtered.filter(project => !project.is_general_expense);
    }
            ...
            <div className="mb-4">
              ...
              {/* 全体経費表示チェックボックス */}
                <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={showGeneralExpense}
                    onChange={(e) => setShowGeneralExpense(e.target.checked)}
                    className="rounded"
                  />
                  全体経費を表示
                </label>
