...
def create_user(
    ...
    try:
        ...
        db.add(new_user)
        db.commit()
        db.refresh(new_user)  # 新しいユーザーのIDを取得
        
        # 全体経費工事を自動作成
        company_db = get_company_session(current_user.company_id)
        try:
            general_expense_project = company_models.Project(
                user_id=new_user.id,
                project_code="経費",
                name=f"{new_user.name}の全体経費",
                is_general_expense=True,
                status="active",
                contract_amount=0,
                period=None  # 期は不要
            )
            company_db.add(general_expense_project)
            company_db.commit()
        finally:
            company_db.close()
