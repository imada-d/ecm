...
# === 帳票テンプレート設定 ===
@router.get("/print-template", response_model=schemas.PrintTemplate)
def get_print_template(
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user)
):
    """帳票テンプレート設定を取得"""
    template = db.query(company_models.PrintTemplate).first()
    
    if not template:
        # デフォルト設定を作成
        default_config = {
            "columns": [
                {"field": "project_code", "label": "工事番号", "enabled": True},
                {"field": "name", "label": "工事内容", "enabled": True},
                {"field": "client_name", "label": "受注先", "enabled": True},
                {"field": "contract_amount", "label": "受注金額", "enabled": True},
                {"field": "paid_amount", "label": "収入金額", "enabled": True}
            ],
            "showTotal": True
        }
        import json
        template = company_models.PrintTemplate(
            template_name="工事台帳",
...
def update_print_template(
    template_update: schemas.PrintTemplateUpdate,
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user)
):
    """帳票テンプレート設定を更新（管理者のみ）"""
    ...
    if template_update.config:
        template.config = template_update.config
    
    template.updated_at = datetime.now()
    db.commit()
    db.refresh(template)
    
    return {"message": "帳票設定を更新しました", "template": template}
